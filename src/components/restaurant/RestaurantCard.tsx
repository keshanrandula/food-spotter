...
import { Heart, MapPin, Sparkles, Phone, ChevronRight, Camera } from 'lucide-react';
...
interface RestaurantCardProps {
  ...
  onOpenGallery?: (photos: string[], name: string, startIndex: number) => void;
  isSaved?: boolean;
}
  ...
  return (
    <div className="group relative bg-white border border-warm-200/90 rounded-3xl overflow-hidden shadow-savor hover:shadow-savor-hover transition-all duration-300 flex flex-col h-full">
      ...
      <div className="relative h-56 w-full overflow-hidden bg-stone-900">
        ...
        {/* Photo gallery click overlay */}
        {onOpenGallery && restaurant.photos.length > 0 && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onOpenGallery(restaurant.photos, restaurant.name, 0);
            }}
          ...
          >
            <span className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 backdrop-blur-md text-white text-xs font-semibold border border-white/20">
              <Camera size={13} /> {restaurant.photos.length} Photos
            </span>
          </button>
        )}
        ...
        <div>
          ...
          <div className="rounded-2xl bg-[#fbf5ed] border border-[#f3e4d4] p-4 text-xs space-y-2">
            ...
            <p className="text-stone-700 leading-relaxed italic">
              "{restaurant.aiReviewSummary || restaurant.aiSummary?.overallSummary || 'Diners rave about the freshly prepared food, cozy ambience, and warm hospitality.'}"
            </p>
          </div>
          ...
          {restaurant.menuDishes && restaurant.menuDishes.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {restaurant.menuDishes.slice(0, 3).map((dish, dIdx) => (
                <span
                  key={dIdx}
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-stone-100 text-stone-700 border border-stone-200/70 text-[11px] font-semibold"
                >
                  <span>🍽️</span>
                  <span>{dish}</span>
                </span>
